...
module.exports = async (req, res) => {
  ...
  if (!apiKey) {
    res.status(500).json({ error: 'No API key found', detail: 'process.env.ANTHROPIC_API_KEY is empty or undefined on this deployment.' });
    return;
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      res.status(502).json({ error: 'Upstream API error (status ' + response.status + ')', detail: errText.slice(0, 300) });
      return;
    }
